...
// Function to delete a favorite for a user
function deleteFavorite($userID, $symbol) {
    $db = openDatabase();

    $query = "DELETE FROM user_favourites WHERE UserID = :userID AND favourite = :symbol";
...
// Handle the delete favorites request
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($sessionID) && !empty($symbol)) {
        ...
        if ($userID !== null) {
            // Delete favorite for the user
            $result = deleteFavorite($userID, $symbol);
