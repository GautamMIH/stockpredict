...
right_share_tab = WebDriverWait(driver, 10).until(
        EC.element_to_be_clickable((By.XPATH, '//a[text()="Mutual Fund"]'))
    )
...
try:
    ...
    while True:
        ...
        table = WebDriverWait(driver, 10).until(
            EC.presence_of_element_located((By.ID, 'myTableEmf'))
        )
        ...
        try:
            next_page_link = WebDriverWait(driver, 15).until(EC.element_to_be_clickable((By.ID, 'myTableEmf_next')))
    ...
    with open('jsondata/mutualfund.json', 'w') as json_file:
