...
right_share_tab = WebDriverWait(driver, 10).until(
        EC.element_to_be_clickable((By.XPATH, '//a[text()="Bonds/Debentures"]'))
    )
...
try:
    ...
    while True:
        ...
        table = WebDriverWait(driver, 10).until(
            EC.presence_of_element_located((By.ID, 'myTableEbd'))
        )
        ...
        for row in rows:
            ...
            if len(cells) >= 8:  # Ensure that there are at least three cells
                ...
                OpeningDate = cells[5].text.strip()
                ClosingDate = cells[6].text.strip()
                LClosingDate = cells[7].text.strip()
                ListingDate = cells[8].text.strip()
                IssueManager = cells[9].text.strip()
        ...
        try:
            next_page_link = WebDriverWait(driver, 15).until(EC.element_to_be_clickable((By.ID, 'myTableEbd_next')))
    ...
    with open('jsondata/debentures.json', 'w') as json_file:
